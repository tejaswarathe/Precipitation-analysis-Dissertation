...
r95PercentileInterpolated = np.load('Calculated Data/95Percentile_BasePeriod.npy')
...
r95pTOT = {'historical' : np.zeros([40,53,113]),
...
def calculateR95pTOT(precip, time_period, year):
    global r95PercentileInterpolated, r95pTOT
    for i in range(53):
        for j in range(113):
            if(precip[i][j] > r95PercentileInterpolated[i][j]):
                r95pTOT[time_period][year][i][j] += precip[i][j]
    ...
    for file in data_files:
        ...
        for y in range(len(ppt)//365):
            ...
            for matrix in oneYearPPT:
                ...
                calculateR95pTOT(new_matrix, time_period, year)
...
print(r95pTOT[time_period])


# save to npy file
np.save('Calculated Data/Historical/R95pTOT_historical.npy', r95pTOT[time_period])
