...
R10mm = {'historical': np.zeros([40,53,113]),
...
def calculateR10mm(precip, time_period, year):
    global R10mm
    for i in range(53):
        for j in range(113):
            if(precip[i][j] >= 10):
                R10mm[time_period][year][i][j] += 1
    ...
    for file in data_files:
        ...
        for y in range(len(ppt)//365):
            ...
            for matrix in oneYearPPT:
                ...
                calculateR10mm(new_matrix, time_period, year)
...
print(R10mm[time_period])


# save to npy file
np.save('Calculated Data/Historical/R10mm_historical.npy', R10mm[time_period])
