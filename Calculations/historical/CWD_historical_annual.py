...
cwd_matrix = {'historical': np.zeros([40,53,113]),
'period1': np.zeros([40,53,113]),
'period2': np.zeros([40,53,113])
}

current_cwd_matrix = {'historical': np.zeros([40,53,113]),
'period1': np.zeros([40,53,113]),
'period2': np.zeros([40,53,113])
}
...
def calculateCWD(precip, time_period, year):
    global cwd_matrix
    for i in range(53):
        for j in range(113):
            if(precip[i][j] >= 1):
                current_cwd_matrix[time_period][year][i][j] += 1
            else:  
                cwd_matrix[time_period][year][i][j] = max(current_cwd_matrix[time_period][year][i][j], cwd_matrix[time_period][year][i][j])
                current_cwd_matrix[time_period][year][i][j] = 0
            cwd_matrix[time_period][year][i][j] = max(current_cwd_matrix[time_period][year][i][j], cwd_matrix[time_period][year][i][j])
    ...
    for file in data_files:
        ...
        for y in range(len(ppt)//365):
            ...
            for matrix in oneYearPPT:
                ...
                calculateCWD(new_matrix, time_period, year)
...
print(cwd_matrix[time_period])


# save to npy file
np.save('Calculated Data/Historical/cwd_historical_annual.npy', cwd_matrix[time_period])
