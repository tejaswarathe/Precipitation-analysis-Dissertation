...
def calculateCDD(precip, time_period):
    ...
    for i in range(53):
        for j in range(113):
            if(precip[i][j] < 1):
                current_cdd_matrix[time_period][i][j] += 1
            else:  
                cdd_matrix[time_period][i][j] = max(current_cdd_matrix[time_period][i][j], cdd_matrix[time_period][i][j])
                current_cdd_matrix[time_period][i][j] = 0
            cdd_matrix[time_period][i][j] = max(current_cdd_matrix[time_period][i][j], cdd_matrix[time_period][i][j])
...
with os.scandir(data) as data_files:
    for file in data_files:
        ...
        for matrix in ppt:
            ...
            calculateCDD(new_matrix, time_period)
...
# save to npy file
np.save('Calculated Data/Historical/cdd_historical.npy', cdd_matrix[time_period])
