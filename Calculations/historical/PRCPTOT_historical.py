...
PRCPTOT = {'historical': np.zeros([40,53,113]),
...
def calculatePRCPTOT(precip, time_period, year):
    global PRCPTOT
    for y in range(53):
        for x in range(113):
            if(precip[y][x] >= 1):
                PRCPTOT[time_period][year][y][x] += precip[y][x]
    ...
    for file in data_files:
        ...
        for y in range(len(ppt)//365):
            ...
            for matrix in oneYearPPT:
                ...
                calculatePRCPTOT(new_matrix, time_period, year)
...
print(PRCPTOT[time_period])


# save to npy file
np.save('Calculated Data/Historical/PRCPTOT_historical.npy', PRCPTOT[time_period])
