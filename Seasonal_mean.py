...
mean_precipitation = {'Pre Monsoon': np.zeros([400,450]),
'Monsoon': np.zeros([400,450]),
'Post Monsoon': np.zeros([400,450]),
'Winter': np.zeros([400,450])}
...
def calculateMeanPrecipitation(season):
    global mean_precipitation, total_precipitation, numOfdays
    print(numOfdays[season])
    mean_precipitation[season] = np.true_divide(total_precipitation[season], numOfdays[season])
...
calculateMeanPrecipitation(season)
...
# Plot Data
cs = map.pcolor(y, x, mean_precipitation[season], cmap='gist_rainbow', shading='auto', vmin=minmin, vmax=maxmax)
...
plt.suptitle('Mean precipitation')
plt.show()
