...
R20mm = {'historical': np.zeros([53,113]),
...
def calculateR20mm(precip, time_period):
    global R20mm
    for i in range(bottomLat, topLat):
        for j in range(leftLon, rightLon):
            if(matrix[i][j]*24 >= 20):
                R20mm[time_period][i-bottomLat][j-leftLon] += 1
...
with os.scandir(data) as data_files:
    for file in data_files:
        ...
        for matrix in ppt:
            calculateR20mm(matrix, time_period)
...
# Plot Data
cs = map.pcolor(x, y, R20mm[time_period], cmap=reversed_color_map, shading='auto')
